...
bool nextFiles(SdFs* sd) {
    ...
    FsFile audioFile = sd->open(audioFileName, O_WRONLY | O_CREAT | O_EXCL);
    ...
    FsFile timestampFile = sd->open(timestampFileName, O_WRONLY | O_CREAT | O_EXCL);
...
bool writeWAVData(SdFs* sd, WriteWAVParams *params) {
    ...
    FsFile file = sd->open(audioFileName, O_WRONLY | O_APPEND | O_EXCL);
...
bool writeButtonData(SdFs* sd, ButtonEvent* event) {
    ...
    FsFile file = sd->open(timestampFileName, O_WRONLY | O_APPEND | O_EXCL);
    bool error = file.printf("%ld %ld\n", event->pressTime - startTimestamp, event->releaseTime - startTimestamp) == 0;
    file.close();
    if (error) {
        Serial.println("!! Error writing button data to file");
        return false;
    }
    return true;
}
